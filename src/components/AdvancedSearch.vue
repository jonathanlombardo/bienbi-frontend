...
export default {
	...
	created() {
		...
		store.services.forEach((service) => {
			this.activeFilter["service-" + service.id] =
				this.$route.query["service-" + service.id];
		});
	},
	...
	<section class="p-5 jumbo d-flex flex-column">
		<div>
			<h1 class="text-center mb-4 fw-bold">
				Cerca l'appartamento che fa al caso tuo
			</h1>
		</div>
			...
			<div class="row">
				...
				<div class="col p-3">
					...
					<input
						...
						id="bathrooms"
						@input="newFilter()"
						v-model="activeFilter.bathrooms"
					/>
				</div>
				<div class="col p-3">
					<div class="servizi-img p-3">
						<img src="/img/mq.png" alt="casa" />
						<label for="square_meeters" class="form-label ms-2"
							>Metri quadri</label
						>
					</div>
					<input
						...
						id="square_meeters"
						@input="newFilter()"
						v-model="activeFilter.square_meeters"
					/>
				</div>

				<div class="col-12">
					<div v-for="service of store.services" class="input-group">
						<input
							type="checkbox"
							:id="'service-' + service.id"
							@change="handleServiceChange(service.id)"
							:checked="activeFilter['service' + service.id]"
						/>
